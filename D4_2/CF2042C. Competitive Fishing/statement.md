...
The second line contains a string $s$, consisting of exactly $n$ characters. The $i$-th character is either `0` (denoting that the $i$-th fish was caught by Alice) or `1` (denoting that the $i$-th fish was caught by Bob).

Additional constraint on the input: the sum of $n$ over all test cases does not exceed $2 \cdot 10^5$.

## Output

For each test case, print a single integer — the minimum number of groups you have to split the fishes into; or $-1$ if it is impossible.
